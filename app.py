...
# Define Dash callback to update the graph when the button is clicked
@dash_app.callback(
    Output("graph", "figure"),
...
def update_graph(n_clicks):
    # if n_clicks is None:
    #     return go.Figure()  # Return empty figure if no clicks

    # Retrieve processed data from the session
    data = session.get('processed_data')
...
def handle_input():
    ...
    # Save processed data in session
    session['processed_data'] = processed_data.to_dict()

    # Redirect to the page with the embedded graph
    return redirect(url_for('char_disp'))

@server.route('/char_disp')
def char_disp():
    current_year = datetime.now().year
    # Render a template that contains the embedded Dash graph and the home button
    return render_template('char_disp.html', current_year=current_year)
